// amps_sow_and_subscribe_with_oof.cpp
//
...
// * Connect to AMPS
// * Logon
// * Query messages from the SOW topic with a simple filter. The subscription
//   requests OOF messages.
// * Output the messages to the console. The program continues
//   to receive update messages after the SOW query finishes. Notice
//   that the program handles OOF messages by checking the command returned
//   in the message.
//
...
int main (void)
{
  ...
  const char* uri = "tcp://sample_user@:127.0.0.1:9027/amps/json";

  // Create a client with the name "exampleSowAndSubscribe"

  AMPS::Client ampsClient("exampleSowAndSubscribeWithOOF");
  ...
  try
  {
    ...
    // After the query is done, the program receives new messages
    // on a subscription. When a previously received message no longer
    // matches the subscription, the program receives an Out of Focus
    // message.
    ...
    AMPS::MessageStream stream = ampsClient.sowAndSubscribe(
                                   "messages-sow",
                                   0,      // Timeout
                                   "/messageNumber % 10 = 0 AND (/OptionalField IS NULL OR /OptionalField <> 'ignore_me')", // Filter
                                   0,       // Batch Size
                                   true);  // OOF enabled
