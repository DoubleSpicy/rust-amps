...
//
// This sample uses automatic acknowledgement of the messages in the queue.
// with this setting, the client will handle acknowledgement for the previous
// message each time the MessageStream returns a new message. (See the
// Developer Guide for full details.)
//
...
int main (void)
{
  ...
  try
  {
    // enable automatic acknowledgement on queue subscriptions
    ampsClient.setAutoAck(true);
    ...
    AMPS::MessageStream stream = ampsClient.subscribe("sample-queue");
